...
function goPerson(p) {
  const [bookId, ch] = (p.first || '01:1').split(':')
  go(`/brp/${bookId}/${ch || 1}`)
}
function goPlace(p) {
...
function verseAddr(v) {
  const b = index.value?.books?.[v.bookIndex]
  return b ? `${b.zh} ${v.chapter}:${v.verse}` : ''
}
...
const GROUPS = computed(() => {
  ...
  if (!r) return []
  const defs = [
    { key: 'persons', icon: '👤', label: '人物', items: r.persons, act: goPerson, actIcon: '📖', actTitle: '查看首处经文' },
              ...
              <section v-for="g in GROUPS" :key="g.key" class="sp-group">
                ...
                <ul class="sp-list">
                  <li v-for="item in g.items" :key="item.raw.id">
                    <button class="sp-item" :title="g.actTitle" @click="g.act(item.raw)">
