...
**当前覆盖：56 卷注释全部章节完整（ok=56、mismatch=0）**。竖排分栏 3 卷（54-55 提摩太、56 提多、57 腓利门）已支持：竖排重建管线（`vertical_rebuild`：剔除页码行、栏标题逐字块白名单重组；`parse_vertical`：按书名分流多卷，54-55 一个文件产出两卷）。57 腓利门实为横排且无章节标题，走单章书兜底（全书作第 1 章）。
