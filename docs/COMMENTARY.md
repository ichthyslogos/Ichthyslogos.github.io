...
**当前覆盖：56 卷注释全部章节完整，小节 ref 连续无缺口（audit_commentary.py 复扫通过）**。
解析含三层 ref 保障：新经文块自动开新小节（避免 ref 覆盖）、注释中单节引文识别、后处理（倒退小节并入前段 + 相邻缺口补全）。
竖排分栏 3 卷（54-55 提摩太、56 提多、57 腓利门）已支持（竖排重建管线）；57 腓利门实为横排无章节标题，走单章书兜底。

普查脚本：`python scripts/commentary/audit_commentary.py`（章节完整性 + ref 覆盖缺口 + 缺失卷清单）

| 缺失项 | 说明 |
|---|---|
| **整卷缺失（10 卷，素材无）** | 48 加拉太书、49 以弗所书、50 腓立比书、51 歌罗西书、52 帖撒罗尼迦前书、53 帖撒罗尼迦后书、62 约翰一书、63 约翰二书、64 约翰三书、65 犹大书——前端显示"本卷暂无注释" |
| **诗篇 101-150 篇** | 素材 19a-d 仅覆盖 1-100 篇，诗篇 101-150 显示"本卷暂无注释" |
| 英文 EPUB | `Unabridged Matthew Henrys…epub` 为英文全本，明确忽略（只做中文版译注） |
